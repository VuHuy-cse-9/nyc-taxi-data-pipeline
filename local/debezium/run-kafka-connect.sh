...
case $cmd in
    register_connector)
        if [[ -z "$2" ]]; then
            ...
            curl -i -X POST -H "Accept:application/json" -H 'Content-Type: application/json' http://connect:8083/connectors -d @$2
        fi
        ;;
    remove_connector)
        if [[ -z "$2" ]]; then
            ...
            curl -i -X DELETE -H "Accept:application/json" http://connect:8083/connectors/$2
        fi
        ;;
    *)
