...
with DAG(
    ...
    run_ingestion_batch_processor = DockerOperator(
        task_id="run_ingestion_yellow_taxi_job",
        image="doan-batch_processor:latest",
        container_name="ingestion_yellow_taxi_container",
        api_version="auto",
        auto_remove="force",
    ...
    run_warehouse_batch_processor = DockerOperator(
        task_id="run_warehouse_yellow_taxi_job",
        image="doan-batch_processor:latest",
        container_name="warehouse_yellow_taxi_container",
        api_version="auto",
        auto_remove="force",
        command="uv run -m datawarehouse.yellow_taxi",
        environment=common_env,
        docker_url="unix://var/run/docker.sock",  # adjust if remote
        network_mode="doan_nyc_network",  # change if using custom network
    )
    ...
    run_create_partition_job = DockerOperator(
        task_id="run_create_partition_yellow_taxi_job",
        image="doan-batch_processor:latest",
        container_name="create_partition_yellow_taxi_container",
        api_version="auto",
        auto_remove="force",
    ...
    run_datamart_batch_processor = DockerOperator(
        ...
        image="doan-batch_processor:latest",
        container_name="datamart_yellow_taxi_container",
        api_version="auto",
        auto_remove="force",
