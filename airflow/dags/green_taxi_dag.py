...
with DAG(
    ...
    run_warehouse_batch_processor = DockerOperator(
        task_id="run_warehouse_green_taxi_job",
        image="doan-batch_processor:latest",
        container_name="warehouse_green_taxi_container",
        api_version="auto",
        auto_remove="force",
        command="uv run -m datawarehouse.green_taxi",
        environment=common_env,
        docker_url="unix://var/run/docker.sock",  # adjust if remote
        network_mode="doan_nyc_network",  # change if using custom network
    )
    ...
    run_create_partition_job = DockerOperator(
        task_id="run_create_partition_green_taxi_job",
        image="doan-batch_processor:latest",
        container_name="create_partition_green_taxi_container",
        api_version="auto",
        auto_remove="force",
    ...
    run_datamart_batch_processor = DockerOperator(
        task_id="run_datamart_green_taxi_job",
        image="doan-batch_processor:latest",
        container_name="datamart_green_taxi_container",
        api_version="auto",
        auto_remove="force",
        command="uv run -m datamart.green_taxi",
        environment=common_env,
        docker_url="unix://var/run/docker.sock",  # adjust if remote
        network_mode="doan_nyc_network",  # change if using custom network
    )
